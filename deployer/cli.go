...
func commonFlags(requireDeployment bool) []cli.Flag {
	flags := []cli.Flag{
		...
		&cli.StringFlag{Name: "postgres-password", Usage: "legacy Citus PostgreSQL password used for source export and rollback"},
		...
		&cli.BoolFlag{Name: "disable-legacy-citus", Usage: "Render Citus disabled; use only after verified YugabyteDB cutover and backup acceptance"},
...
func commandOptions(cmd *cli.Command) Options {
	return Options{
		...
		DisableLegacyCitus:        cmd.Bool("disable-legacy-citus"),
