...
func renderComposeEnv(prepared *preparedOptions) string {
	lines := []string{
		...
		"",
		"YUGABYTE_HOST=" + prepared.YugabyteHost,
		fmt.Sprintf("YUGABYTE_PORT=%d", prepared.YugabytePort),
		"YUGABYTE_USER=" + prepared.YugabyteUser,
		"YUGABYTE_PASSWORD=" + prepared.YugabytePassword,
		"YUGABYTE_DB=" + prepared.YugabyteDatabase,
...
func renderHelmValues(prepared *preparedOptions, configs map[string]string) string {
	...
	lines := []string{
		...
		`  provider: "yugabyte"`,
		"  yugabyte:",
		fmt.Sprintf("    host: %q", prepared.YugabyteHost),
		fmt.Sprintf("    port: %d", prepared.YugabytePort),
		fmt.Sprintf("    user: %q", prepared.YugabyteUser),
		fmt.Sprintf("    password: %q", prepared.YugabytePassword),
		fmt.Sprintf("    database: %q", prepared.YugabyteDatabase),
		fmt.Sprintf("    sslMode: %q", prepared.YugabyteSSLMode),
		"    colocation: false",
		"",
		"yugabyteInit:",
		"  enabled: true",
